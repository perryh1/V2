...
“”“Fetch latest ERCOT West Hub real-time LMP via gridstatus. Falls back to simulation.”””
try:
import gridstatus
...
```
# Simulated price — realistic diurnal pattern for West Texas
now    = datetime.utcnow()
...
“”“Fetch current weather for Midland TX from Open-Meteo (free, no key required).”””
url = (
...
“wind_mph”:      14.0,
“solar_w_m2”:    550.0,
“source”:        f”Simulated ({e})”,
}

@st.cache_data(ttl=300)
def get_weather_history() -> pd.DataFrame:
“”“Fetch 24-hr hourly history for sparkline charts.”””
now   = datetime.utcnow()
...
“”“GHI (W/m²) → Plant output (MW). ~85% derating at STC 1000 W/m².”””
cf = min(solar_w_m2 / 1000.0, 1.0) * 0.85
return round(SOLAR_CAP_MW * cf, 2)

def calc_wind_output(wind_mph: float) -> float:
“”“Simplified power curve: cut-in 7 mph, rated 30 mph, cut-out 55 mph.”””
...
if price < 0:
    ...
    # Price > MINING_REV_MWH — SHUT MINERS, full grid dispatch + battery discharge
...
if price < 0:
    status = "⚠️ NEGATIVE PRICE — Charge & Mine Mode"
elif price > MINING_REV_MWH:
    ...
    <div class="ticker-bar">
        ...
        <div>
            ...
            <div style="font-size:11px; color:#475569; margin-top:8px;">
                ⛏️ Mining Breakeven: <strong style="color:#94a3b8;">${MINING_REV_MWH:.2f}/MWh</strong>
            </div>
        </div>
        <div style="margin-left:auto; text-align:right;">
...
with g1:
    st.plotly_chart(make_gauge(solar_mw, SOLAR_CAP_MW, "☀️ Solar Output", "MW", "#f59e0b"),
                    use_container_width=True, config={"displayModeBar": False})
with g2:
    st.plotly_chart(make_gauge(wind_mw, WIND_CAP_MW, "🌬️ Wind Output", "MW", "#3b82f6"),
                    use_container_width=True, config={"displayModeBar": False})
with g3:
    st.plotly_chart(make_gauge(miner_active_mw, MINER_MW, "⛏️ Miner Load", "MW", "#8b5cf6"),
                    use_container_width=True, config={"displayModeBar": False})
with g4:
    st.plotly_chart(make_gauge(battery_soc, 100, "🔋 Battery SOC", "%", "#06b6d4"),
                    use_container_width=True, config={"displayModeBar": False})

# Weather pill row
p1, p2, p3, p4, p5 = st.columns(5)
pills = [
    (p1, f"🌡️ Temp: <strong>{weather['temperature_f']}°F</strong>"),
    (p2, f"💨 Wind: <strong>{weather['wind_mph']} mph</strong>"),
    (p3, f"☀️ GHI: <strong>{weather['solar_w_m2']} W/m²</strong>"),
    (p4, f"🌞 Solar CF: <strong>{solar_mw/SOLAR_CAP_MW*100:.1f}%</strong>"),
    (p5, f"🌬️ Wind CF: <strong>{wind_mw/WIND_CAP_MW*100:.1f}%</strong>"),
]
...
with col_a:
    ...
    st.markdown(
        ...
        <div class="rev-card">
            ...
            <div class="card-subtitle">🌞🌬️ Renewable Only — Pure Grid Sales</div>
...
with col_b:
    ...
    st.markdown(
        ...
        <div class="rev-card">
            ...
            <div class="card-subtitle">⛏️ Mining Only — 35 MW, 24/7 Regardless of Price</div>
...
with sp1:
    ...
    fig_s.update_layout(
        title={"text": "☀️ Solar Output (MW)", "font": {"size": 13, "color": "#e2e8f0"}},
        paper_bgcolor="#0a0e1a", plot_bgcolor="#0f172a",
        font={"family": "Inter", "color": "#94a3b8"},
...
with sp2:
    ...
    fig_w.update_layout(
        title={"text": "🌬️ Wind Output (MW)", "font": {"size": 13, "color": "#e2e8f0"}},
        paper_bgcolor="#0a0e1a", plot_bgcolor="#0f172a",
        font={"family": "Inter", "color": "#94a3b8"},
...
# ─────────────────────────────────────────────────────────────────────────
# SENSITIVITY ANALYSIS — Revenue vs Price
# ─────────────────────────────────────────────────────────────────────────
...
matrix_df = pd.DataFrame({
    ...
    "Battery":          ["⬇️ Charging  (60 MW)", "Standby", "⬆️ Discharging  (60 MW)"],
...
st.markdown(
    ...
    <div style="text-align:center; padding:24px; color:#334155; font-size:11px;
        ...
        WEST TEXAS RENEWABLE PORTFOLIO DASHBOARD &nbsp;·&nbsp;
        ERCOT WEST HUB &nbsp;·&nbsp;
        MIDLAND TX 32.0°N 102.1°W &nbsp;·&nbsp;
...
with st.sidebar:
    ...
    specs = [
        ("☀️ Solar", f"{SOLAR_CAP_MW:.0f} MW"),
        ("🌬️ Wind", f"{WIND_CAP_MW:.0f} MW"),
        ("⛏️ Miners", f"{MINER_MW:.0f} MW"),
